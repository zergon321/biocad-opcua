...
func (ctl *controller) handleWebsocketUpgradeError(err error) {
	if err != nil {
		ctl.logger.Println("Couldn't upgrade connection to websocket:", err)
	}
}

func (ctl *controller) handleWebsocketSendMessageError(err error) {
	if err != nil {
		ctl.logger.Println("Couldn't send a message through websocket:", err)
	}
}
